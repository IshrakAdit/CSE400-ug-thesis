"""
Adaptive Hybrid FT Engine - rewritten, fixed, and improved.
...
- Expected-value core: EV = ΔS * sla_penalty - cost
"""
...
class OnlineLearner:
    ...
    def update(self, m: MetricRecord):
        a = m.action
        # Update effectiveness: success -> move toward 1.0, failure -> move toward 0.0
        ...
        # Update cost: use extra_cost if present, otherwise normalize overhead seconds as minutes -> $ proxy
...
class KubernetesOrchestrator:
    def __init__(self, namespace: str):
        ...
        if self.enabled:
            # Try lazy import; if fails, remain disabled
            try:
    ...
    def replicate(self, task_id: str, replicas: int, anti_affinity: bool = True) -> str:
        if not self.enabled:
            ...
            logger.info(s)
            return s
        # TODO: implement real ReplicaSet/Job creation
...
class FaultToleranceEngine:
    def __init__(self, learner: OnlineLearner):
        ...
        # stable action preference used for deterministic tie-breaking
    ...
    def _estimate_delta_success(self, action: ActionType, req: DecideRequest, base_risk: float) -> float:
        """
        Estimate ΔS ∈ [0,1] for taking `action`.
        Blend of learned prior and heuristic 'adj' that depends on risk, criticality, progress, time pressure.
        """
        base_prior = float(self.learner.effectiveness.get(action, 0.0))
        # time pressure: 0 when far, 1 when immediate
        ...
        # Heuristic adj per action
        if action == ActionType.replicate:
            # replication most helpful under high risk & critical tasks & time pressure
            adj = 0.25 + 0.75 * base_risk
            adj *= (0.4 + 0.6 * c)
            adj *= (0.5 + 0.5 * time_pressure)
        elif action == ActionType.checkpoint:
            # checkpoints protect remaining work; more useful early (low progress) and when spot eviction is likely
            adj = 0.20 + 0.80 * base_risk
            adj *= (0.4 + 0.6 * (1.0 - p))
            adj *= (0.5 + 0.5 * max(spot, (1.0 - rel)))
        elif action == ActionType.migrate:
            # migration helps when node reliability is low and some time remains
            adj = 0.15 + 0.85 * base_risk
            adj *= (0.5 + 0.5 * (1.0 - rel))
            adj *= (0.4 + 0.6 * time_pressure)
        elif action == ActionType.resubmit:
            # resubmit is cheap; better earlier in run when more can be retried
    ...
    def _select_best_action(
        ...
        """
        Deterministically pick the best action with tie-breaking:
        1) highest EV
        2) highest ΔS
        3) lowest cost
        4) stable priority order
        Also apply guardrails (avoid resubmit if high risk & heavy time pressure).
        """
        # Apply guardrail: if risk high and near deadline avoid resubmit
        if risk >= 0.3 and time_pressure > 0.6:
            # deprioritize resubmit by reducing its EV slightly
            if ActionType.resubmit in evs:
                evs = evs.copy()
                evs[ActionType.resubmit] = evs.get(ActionType.resubmit, 0.0) - 1e-6

        # Find max EV
        max_ev = max(evs.values())
        # If best EV <= 0 -> no_action
        if max_ev <= 0.0:
            return ActionType.no_action

        # Filter candidates with EV == max_ev (within tiny epsilon)
        eps = 1e-9
        ...
        candidates.sort(
            key=lambda a: (
                -float(deltas.get(a, 0.0)),         # prefer larger delta
                float(costs.get(a, float("inf"))),  # prefer smaller cost
                self._action_priority.index(a)      # stable action order
            )
        )
        chosen = candidates[0]
        return chosen
    ...
    def _make_params(self, action: ActionType, req: DecideRequest, R: float) -> Dict[str, Any]:
        ...
        if action == ActionType.checkpoint:
            # interval shorter when risk is high; clamp 30..600
    ...
    def decide(self, req: DecideRequest) -> Tuple[ActionType, Dict[str, float], float, Dict[str, Any], str]:
        ...
        for a in actions:
            ...
            deltas[a] = delta
            # choose cost: prefer provided positive cost, otherwise learner fallback
        ...
        # Convert evs keys to strings for JSON friendliness
...
# FastAPI App
# ---------------------------
app = FastAPI(title="Adaptive Hybrid FT Engine", version="1.1.0")
...
async def ingest_metrics(m: MetricRecord):
    # Validate action is known and update learner
